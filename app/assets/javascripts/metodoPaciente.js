...
function getConsultas(form) {
    ...
    $.get('/../api/expediente/buscar/consultas/' + expediente, function(data, status) {
        ...
        if (consultas == "") {
            $('#consultas_list').html("<p class='red-text center'> El numero de expediente no se encuentra registrado</p>")
        } else {
...
function seleccionarConsultas(NoExpediente, NombrePaciente, Tratamiento, Psicologo, Avances, TotalSesiones) {
    ...
    var num = document.getElementById('NoExpediente').value;
    if (num == "") {
        document.getElementById('body_consultas').style.display = 'none';
        document.getElementById('NombrePaciente').placeholder = 'Paciente';
    }
}